...
- **Confidence:** medium
- **Maturity:** experimental
- **Alternatives and tradeoffs:** Existing subscriptions avoid local setup and usually provide a higher quality ceiling but send approved content to a vendor boundary. Local weights remove per-token fees and can narrow data movement, but add hardware contention, slower inference, quantization loss, runtime/tool integration, and maintenance.
- **Subscription / licensing impact:** Open-weight licenses permit the described evaluation subject to their exact terms; gpt-oss also carries a usage policy. No new subscription is required, but hardware, power, operator time, and any optional hosting are real costs.
...
- **Verified:** 2026-07-26
- **Product / plan / version:** Runpod public Pod list observed 2026-07-26: 24 GB A5000 USD 0.27/hour, L4 USD 0.39/hour, RTX 3090 USD 0.50/hour, RTX 4090 USD 0.69/hour; 48 GB A40 USD 0.44/hour, A6000 USD 0.53/hour, 6000 Ada USD 0.84/hour, and L40S USD 0.99/hour. Prices are dynamic and exclude storage, network, idle time, and administration.
- **Confidence:** medium
- **Maturity:** experimental
...
- **Use when:** A later Gate has at least 48 GB available accelerator/unified memory, a pinned runtime supports the architecture, and an isolated coding/tool pilot can compare the exact quantization with the accepted subscription baseline.
- **Do not use when:** The target is the current 16 GB or 32 GB developer laptop, full advertised context is assumed without memory measurement, or sparse active parameters are mistaken for total weight footprint.
...
- **Confidence:** high
- **Maturity:** experimental
- **Alternatives and tradeoffs:** Qwen2.5-Coder-14B fits a bounded laptop experiment with a lower quality ceiling; Devstral Small 2 has a documented 24 GB GPU path; gpt-oss-20b offers broader reasoning on a smaller footprint; existing subscriptions avoid shared-serving operations.
...
| SRC-114 | OpenAI | https://huggingface.co/openai/gpt-oss-20b/blob/main/USAGE_POLICY | First-party usage-policy text | gpt-oss usage policy on current model revision | 2026-07-26 | Model and open-source routing matrix | Separate usage restrictions that accompany the open-weight release. |
...
| SRC-118 | Microsoft | https://huggingface.co/microsoft/Phi-4-mini-instruct-onnx | Official model and artifact card | Phi-4-mini-instruct ONNX INT4 variants | 2026-07-26 | Model and open-source routing matrix | Parameters, context, setup, function/coding evidence, hallucination warning, and Intel Core Ultra CPU throughput; vendor evidence with incomplete run memory/context. |
...
| SRC-130 | Runpod | https://www.runpod.io/pricing | Official public pricing | Pod GPU list observed 2026-07-26 | 2026-07-26 | Model and open-source routing matrix | Hourly list-price comparison for optional 24/48 GB cloud GPU pilots; prices are dynamic and exclude storage, network, idle time, and administration. |
