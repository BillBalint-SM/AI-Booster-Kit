...
| [`docs/runbooks/branching.md`](../runbooks/branching.md) and [`docs/runbooks/mapper-snapshot.md`](../runbooks/mapper-snapshot.md) | lean branch and mapper-publication rules | runbook evidence; publication still needs approval |
...
| R-10 | Do not commit, push, PR, merge, or publish merely because checks pass. | C:\Users\littl\.codex\AGENTS.md:77-90,109-111; docs/runbooks/mapper-snapshot.md:55-67 | Separates validation from publication authority. | Approval boundary is low-cost relative to rollback risk. | `KEEP` |
...
| R-13 | Run mapper freshness after every documentation-only edit. | docs/runbooks/mapper-snapshot.md:43-53; scripts/check-mapper-freshness.mjs | Intended freshness protection; no value for text-only changes. | Previously produced a non-actionable failure. | `REMOVE_CANDIDATE` — replaced by publication-only check |
