...
| Controller recommendation MVP | `COMPLETE_WITH_LIMIT` | `src/controller/`, controller tests, local CLI | Expand from the bounded Quick Task decision to scenario-aware library recipes. | `implements` Controller layer |
...
| Quick Task Activation Package | `PARTIAL` | Reviewed design and implementation plan under `docs/superpowers/` | Implement and verify the bounded ephemeral package as a separate slice. | `depends_on` Human Checkpoint |
| Agent Framework Library and Recipe Controller v1 | `NOW` / `NOT READY` | Vision, contracts, and first recipe provide the seed | Define catalog, profiles, prerequisites, scenario recognition, rationale, and recipe output. | `depends_on` Controller; `enables` all later Agent tracks |
...
1. the catalog has at least the first light Quick Task formation and bounded
   candidates for research, refinement, implementation, debugging, and
   validation;
...
prerequisites, add scenario recognition, and produce one explainable
recommendation with a stable identity signal.
