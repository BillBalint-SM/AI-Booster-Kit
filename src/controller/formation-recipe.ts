...
import type { DebuggingRecipe, ImplementationRecipe, RefinementRecipe, ResearchRecipe, ValidationRecipe } from "./types.js";
...
export class DebuggingRecipeError extends Error {
  public constructor(field: string, message: string) {
    super(`Debugging recipe rejected: ${field} ${message}.`);
    this.name = "DebuggingRecipeError";
  }
}
...
export async function loadDebuggingRecipe(sourcePath: string): Promise<DebuggingRecipe> {
  return parseDebuggingRecipe(await readFile(sourcePath, "utf8"), sourcePath);
}
...
export function parseDebuggingRecipe(source: string, sourcePath: string): DebuggingRecipe {
  try {
    return parseDebuggingRecipeUnchecked(source, sourcePath);
  } catch (error) {
    if (error instanceof ValidationRecipeError) throw new DebuggingRecipeError(error.field, error.reason);
    throw error;
  }
}

function parseDebuggingRecipeUnchecked(source: string, sourcePath: string): DebuggingRecipe {
  const frontmatter = extractFrontmatter(source, sourcePath);
  const document = parseDocument(frontmatter, { uniqueKeys: true });
  if (document.errors.length > 0) throw new ValidationRecipeError("frontmatter", "contains invalid YAML metadata");
  ...
  const metadata = requireRecord(document.toJS(), "frontmatter");
  requireExactKeys(metadata, rootKeys, "frontmatter");
  requireLiteral(metadata, "recipeId", "bounded-debugging", "recipeId");
  requireLiteral(metadata, "recipeVersion", "0.1.0", "recipeVersion");
  requireLiteral(metadata, "status", "READY", "status");
  requireLiteral(metadata, "formationId", "bounded-debugging", "formationId");
  requireLiteral(metadata, "scenario", "debugging", "scenario");
  requireLiteral(metadata, "weight", "medium", "weight");
  requireLiteral(metadata, "coordination", "sequential", "coordination");
  ...
  requireLiteral(controller, "version", 1, "controller.version");
  requireExactArray(controller, "eligibleComplexities", ["LOW", "MEDIUM"], "controller.eligibleComplexities", "must declare LOW then MEDIUM");
  requireExactArray(controller, "requiredInput", ["symptom", "reproduction", "expected-behavior", "environment"], "controller.requiredInput", "must declare the canonical input sections");
  requireLiteral(controller, "executionBoundary", "LOCAL_ONLY", "controller.executionBoundary");
  requireLiteral(controller, "authority", "RECOMMENDATION_ONLY", "controller.authority");

  const outputContract = requireRecord(metadata.outputContract, "outputContract");
  requireExactKeys(outputContract, outputKeys, "outputContract");
  requireExactArray(outputContract, "requiredSections", ["root-cause-record", "minimal-fix", "regression-evidence"], "outputContract.requiredSections", "must declare the canonical sections");
  requireLiteral(outputContract, "unknownPolicy", "PRESERVE_AS_UNKNOWN", "outputContract.unknownPolicy");
  requireLiteral(outputContract, "resultState", "NOT_STARTED", "outputContract.resultState");

  const acceptance = requireRecord(metadata.acceptance, "acceptance");
  requireExactKeys(acceptance, acceptanceKeys, "acceptance");
  requireExactArray(acceptance, "criteria", ["failure-reproduced", "root-cause-supported", "regression-covered"], "acceptance.criteria", "must declare the canonical criteria");
  requireExactArray(metadata, "evidenceRequirements", ["reproduction-output", "failing-test", "passing-test"], "evidenceRequirements", "must declare the canonical evidence requirements");
  ...
  requireExactKeys(relation, relationKeys, "relations[0]");
  requireLiteral(relation, "kind", "validates", "relations[0].kind");
  requireLiteral(relation, "target", "bounded-implementation", "relations[0].target");

  const recovery = requireRecord(metadata.recovery, "recovery");
  requireExactKeys(recovery, recoveryKeys, "recovery");
  requireExactArray(recovery, "preserve", ["failure-evidence", "pre-fix-state"], "recovery.preserve", "must declare the canonical preserved state");
  requireExactArray(recovery, "stopConditions", ["not-reproduced", "ambiguous-root-cause", "destructive-fix"], "recovery.stopConditions", "must declare the canonical stop conditions");
  ...
  return {
    ...
    acceptance: { criteria: ["failure-reproduced", "root-cause-supported", "regression-covered"] },
    evidenceRequirements: ["reproduction-output", "failing-test", "passing-test"],
    relations: [{ kind: "validates", target: "bounded-implementation" }],
    recovery: {
